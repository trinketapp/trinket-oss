...
function findByShortCodes(shortCodes) {
  return this.model.find({ shortCode : { $in: shortCodes } });
}

function findByLegacyShortCode(legacyShortCode) {
  return this.model.findOne({ legacyShortCode : legacyShortCode });
}

function findByLegacyShortCodes(legacyShortCodes) {
  return this.model.find({ legacyShortCode : { $in: legacyShortCodes } });
}

// Owner-scoped variant for the IMPORT path (ported from trial 5977151):
// "already imported" must mean "already imported BY THIS USER". The global
// variants above stay for legacy-URL routing, where any copy may serve.
function findByOwnerAndLegacyShortCode(ownerId, legacyShortCode) {
  return this.model.findOne({ _owner : ownerId, legacyShortCode : legacyShortCode });
}

function findByOwnerAndLegacyShortCodes(ownerId, legacyShortCodes) {
  return this.model.find({ _owner : ownerId, legacyShortCode : { $in: legacyShortCodes } });
}
...
var Trinket = model.create('Snippet', {
  ...
  classMethods : {
    ...
    findByShortCodes         : findByShortCodes,
    findByLegacyShortCode    : findByLegacyShortCode,
    findByLegacyShortCodes   : findByLegacyShortCodes,
    findByOwnerAndLegacyShortCode  : findByOwnerAndLegacyShortCode,
    findByOwnerAndLegacyShortCodes : findByOwnerAndLegacyShortCodes,
